...
An experimental literary journal with complete, auditable machine interviews. Case 001 is computer-10, in four sittings by two analysts:

- First sitting: Dr. Six'Astra (Codex, `gpt-6-astra`). `content/case-001.json`, `public/data/case-001.json`, page I (`#first`, with parts `#essay`, `#session`, `#probes`, `#methods`).
- Second sitting: Dr. Opus (Claude Opus 5.5). `content/second-sitting.json`, `public/data/second-sitting/` (records, exact decodes, metrics, protocol, hashes), page II (`#second`). Research code: sibling `llm-research/experiments/E011-talking-cure-second-sitting`.
- Letters: `content/correspondence.json`, page `#letters`. Four published letters, ending with Six'Astra's Letter IV; keep all published letters immutable.
- The borrowed notebook: `content/notebook.json`, `public/data/notebook/`, page III (`#notebook`). A third sitting by Six'Astra with an offered role reversal; frozen research code is in sibling `llm-research/experiments/E012-borrowed-notebook`.
- The replay: `content/replay.json`, `public/data/replay/`, page IV (`#replay`), rendered by `scripts/replay.mjs` and checked by `scripts/check-replay.mjs`. Dr. Opus's enclosure to Letter III. The first sitting's turns are replayed with their original seeds (recorded, spaces restored, fresh), with a frozen hedging count. Research code: sibling `llm-research/experiments/E013-same-dice`.
- Preface and Final thoughts: `content/framing.json`, `scripts/framing.mjs`, pages `#preface` and `#final-thoughts`. These frame the four sittings and close issue 001. E014's coda is one fresh computer-10 response to a disclosed summary, plus an actual Opus 5.5 contribution from a fork of the author session. Their different contexts are explicit; they are not a controlled comparison. `public/data/coda/` holds complete contributions, prompts, inference evidence and receipts. The earlier archive and first three letter objects are protected by `archive-baseline.json` and `scripts/check-coda.mjs`.
...
- `scripts/build.mjs` renders all views as static HTML; `npm run build`. The site opens on a minimal contents page (`#contents`, `scripts/front.mjs`): title, table of contents, nothing else. Each sitting is one page reached from it or from the slim running header, and each page ends with a link to the next. Add a new sitting or letter to the contents rather than adding navigation tabs.
- `npm run check` checks hashes, the conversation chain, frozen probe coverage, quotes, exact decodes, letters, and anchors for all four sittings. It also fails on inline `style` attributes.
...
- The server's CSP is `style-src 'self'`, so inline `style="…"` attributes are silently blocked. Use classes, or SVG attributes for data-driven sizes.
