...
An experimental literary journal with complete, auditable machine interviews. Case 001 is computer-10, in two sittings by two analysts:
...
- Letters: `content/correspondence.json`, view 06.
...
- `npm run check` checks hashes, the conversation chain, frozen probe coverage, quotes, exact decodes, letters, and anchors for both sittings.
- The first sitting's `text` fields were decoded with `clean_up_tokenization_spaces=True`, which drops spaces before some quotation marks. Its records stay as recorded. Exact decodes are in `public/data/second-sitting/e010-exact-decode.json`. The second sitting quotes `text_exact`.
